...
spans.forEach(span => span.addEventListener('mouseover', funtion(e) {
    span.classList.add('animated', 'rubberBand')
}))
spans.forEach(span => span.addEventListener('mouseout', funtion(e) {
...
const showRequiredCategory = event => {
    ...
    for(i=0; i<categories.length; i++) {
        if(categories[i].hasAttribute('class')) {
            ...
            categories[i].classList.add('hidenCategory')
        }
    }
